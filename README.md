...
В настоящий момент доступны два варианта установки:
* вручную (процесс подробно изложен в файле `manual_installation.md` в каталоге `installation`),
* в качестве `pod` с помощью менеджера пакетов CocoaPods.
